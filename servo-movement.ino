...
//define servos' ports
const int servo[4][3] = { { 13, 14, 12 }, { 5, 6, 4 }, { 9, 10, 8 }, { 1, 2, 0 } };
/* Size of the robot ---------------------------------------------------------*/
...
/*
  - loop function
   ---------------------------------------------------------------------------*/
char command = '0';   // Store the last command
...
void polar_to_servo(int leg, float alpha, float beta, float gamma) {
  ...
  pwm.setPWM(servo[leg][0], 0, map(alpha, 0, 180, SERVOMIN, SERVOMAX));
  pwm.setPWM(servo[leg][1], 0, map(beta, 0, 180, SERVOMIN, SERVOMAX));
  pwm.setPWM(servo[leg][2], 0, map(gamma, 0, 180, SERVOMIN, SERVOMAX));
}